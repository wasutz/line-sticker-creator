...
1. Inspect the request and workspace for character folders, supplied references, target language, sticker count, tone, audience, theme, and output location. Read every supported character image when a named local character is requested. For a couple pack, inspect both characters and record partner A, partner B, each identity's invariants, their stable relative scale, and relationship tone.
