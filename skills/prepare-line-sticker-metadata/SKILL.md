...
1. Inspect the request and workspace for character folders, supplied references, target language, sticker count, tone, audience, theme, and output location. Read every supported character image when a named local character is requested. For a character variant, record a working variant name plus an inherit-and-delta brief: fixed source identity/style traits and only the requested visible changes. Translate abstract gender presentation into a small, respectful visual delta without body sexualization or stacked stereotypes. For a couple pack, inspect both characters and record partner A, partner B, each identity's invariants, their stable relative scale, and relationship tone.
