...
description: Create, revise, package, or audit static LINE sticker sets for LINE Creators Market from a prompt or a prepared metadata.md brief. Use for LINE sticker concepts, saved-character variants, single-character or two-character couple packs, named characters stored in a local characters library, style-only requests that need original high-utility wording, two-reference requests that take visual direction from one LINE Store set and wording from another, expression lists, art-direction sheets, image generation or editing, submission assets, and preflight validation against LINE's static-sticker creation and review guidelines.
---
...
1. Inspect the workspace for existing characters, sketches, brand rules, fonts, sticker assets, and a user-specified `metadata.md`.
   ...
   - **Research-brief mode:** When the current prompt names a file under `references/research/`, read the entire brief before planning. Treat its dated evidence, character opportunity, wording pool, risks, and handoff as advisory planning input; the user's current prompt and prepared metadata remain authoritative. Record the brief path in output metadata. Never apply a research brief merely because it exists. When the user asks for live character or wording research and sticker creation in one request but supplies no brief, use `research-line-sticker-concepts` first, then consume the resulting file through this mode.
   ...
   - **Character-variant mode:** Use when the user wants a new character derived from a saved character, such as a female-presenting Bunbun, a sibling, or a seasonal version. Treat the saved character as the source identity and write an inherit-and-delta brief before generation. `Inherit` lists the silhouette, anatomy, facial construction, linework, rendering style, and family resemblance that remain fixed. `Delta` lists only the requested distinguishing traits, such as gender presentation, palette accents, markings, hairstyle, clothing, or accessories. Translate an abstract request such as “female” into a small, respectful set of visible traits; avoid relying on body sexualization or stacking unrelated stereotypes. Ask for clarification only when the requested distinction cannot be depicted safely or materially changes the character concept. Give the variant its own name or working label so prompts never confuse it with the source character.
   - **Couple mode:** Use when two characters should appear together in a sticker. Resolve and inspect each named character independently using character-library mode. Before planning, record a compact pairing bible: identity traits for partner A and partner B, their stable relative scale, distinguishing features, and relationship tone. Keep the characters visually distinguishable and preserve those facts across the set. If the user supplies one shared reference image, identify which figure is A and B from the user's description; ask only when that mapping cannot be inferred safely. A couple pack defaults to both partners appearing in every numbered sticker; use a solo composition only when the prompt or approved plan explicitly calls for it.
...
4. Draft a set plan before rendering, or verify and adopt the numbered plan from prepared metadata. Give every sticker a distinct conversational job, readable pose/expression, phrase if any, and composition note. In couple mode, specify partner A's action/expression, partner B's action/expression, their interaction, and which partner speaks when text is not shared; favor unmistakable relationship beats such as affection, teasing, reassurance, apology, celebration, waiting, and playful disagreement. In research-brief mode, trace each adopted character or wording recommendation to the brief and resolve its open questions before rendering. In style-only mode, list the phrases in priority order and check that the set covers both routine replies and emotional reactions without semantic duplicates. In two-reference mode, include the source phrase index and the abstract style traits applied to each design. Avoid near-duplicate poses.
5. Create one approved character/style reference first. In character-library mode, use the library images directly as image references; create a new reference sheet only when the supplied views do not adequately define the character. In character-variant mode, create and approve a standalone variant reference sheet from the source images and inherit-and-delta brief before rendering stickers; save reusable approved references under `characters/<variant-name>/` when the user wants the variant retained. In couple mode, approve one pairing reference that shows both characters together at their stable relative scale before rendering the set. When generating raster art, invoke the `imagegen` skill and include the relevant source or approved variant images in every generation or edit that supports references. For a couple, include references for both characters, identify partner A and B explicitly in the prompt, and restate their interaction. Generate or edit in small batches when that makes consistency easier to inspect. For multi-sticker sheets, require generous gutters and keep every figure, prop, effect, ear, limb, and outline visibly inside its cell; reject a sheet when any artwork touches or crosses an implied cell boundary.
6. Prepare each sticker as an RGB/RGBA PNG on a transparent canvas, with an even-numbered width and height no larger than 370 × 320 px. Keep roughly 10 px of breathing room around visible content. Favor bold silhouettes, large expressions, and short high-contrast lettering that remains legible at chat size.
   ...
   - **Thai typography option — rounded handwritten:** Use `scripts/render_thai_text.py` with Mali Bold, dark warm-gray fill (`#5b4a47`), and a white keyline. The script renders through Pango/HarfBuzz so stacked vowels and tone marks retain correct Thai shaping. Use `|` for an intentional line break and the standard 44 px baseline-to-baseline spacing. Preserve enough vertical headroom around every line for stacked vowels and tone marks; treat any clipped or hard-to-read mark as a failed render. Override the spacing only when the composition or type size clearly requires it. Pass `--verify` to have the script itself fail when rendered ink touches the canvas edge (a proxy for a clipped tone mark or vowel) instead of relying solely on visual inspection. Verify the result on both light and dark chat backgrounds before applying it to the full set.
...
10. Inspect every numbered PNG individually at 1:1 scale, then inspect the full set as a contact sheet against both light and dark chat backgrounds. Confirm that no head, ear, limb, prop, effect, outline, or shadow is cut by a straight edge; no fragment from an adjacent sheet cell remains; roughly 10 px of transparent breathing room surrounds the intended composition; lettering is legible; poses are varied; spelling and metadata align; and `main.png`/`tab.png` honestly represent the set. In character-variant mode, compare the set with both the source and approved variant references: every inherited trait remains recognizable and every required delta stays visible where the pose permits. In couple mode, also confirm both identities, relative scale, speaker attribution, interaction readability, and that overlap does not hide either face or defining feature. A contact sheet alone does not satisfy this check because scaling can hide crop seams.
