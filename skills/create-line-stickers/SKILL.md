...
description: Create, revise, package, or audit static LINE sticker sets for LINE Creators Market from a prompt or a prepared metadata.md brief. Use for LINE sticker concepts, single-character or two-character couple packs, named characters stored in a local characters library, style-only requests that need original high-utility wording, two-reference requests that take visual direction from one LINE Store set and wording from another, expression lists, art-direction sheets, image generation or editing, submission assets, and preflight validation against LINE's static-sticker creation and review guidelines.
---
...
5. Create one approved character/style reference first. In character-library mode, use the library images directly as image references; create a new reference sheet only when the supplied views do not adequately define the character. In couple mode, approve one pairing reference that shows both characters together at their stable relative scale before rendering the set. When generating raster art, invoke the `imagegen` skill and include references for both characters in every couple generation or edit that supports references; identify partner A and B explicitly in the prompt and restate their interaction. Generate or edit in small batches when that makes consistency easier to inspect. For multi-sticker sheets, require generous gutters and keep every figure, prop, effect, ear, limb, and outline visibly inside its cell; reject a sheet when any artwork touches or crosses an implied cell boundary.
