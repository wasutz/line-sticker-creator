...
description: Create, revise, package, or audit static LINE sticker sets for LINE Creators Market. Use for LINE sticker concepts, style-only requests that need original high-utility wording, two-reference requests that take visual direction from one LINE Store set and wording from another, expression lists, art-direction sheets, image generation or editing, metadata, submission assets, and preflight validation against LINE's static-sticker creation and review guidelines.
---
